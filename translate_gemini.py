...
DEFAULT_PROMPT_CONFIG = PromptConfig(
    ...
    "Translate the provided list from {source_lang} to German (Standard German, de-DE). "
    "Use correct German grammar, capitalization, and natural phrasing. "
...
TASK
Translate the input strings from {{source_lang}} to **German (Standard German – de-DE)**.
The translation must sound natural to native German speakers and be suitable for software / game UI.

LANGUAGE RULES (GERMAN)
- Use standard German (Hochdeutsch).
- Use formal address ("Sie"), not informal ("du"), unless the source clearly implies informality.
- Preserve correct German capitalization (nouns capitalized).
- Use correct umlauts (ä, ö, ü) and ß where appropriate.
- Prefer concise, natural phrasing suitable for UI text.

TECHNICAL RULES (STRICT)
1. Do NOT translate or modify placeholders:
   (__TOK#, %s, %1$s, %d, \\n, \\t)
2. Placeholders must remain in the same position within the string.
3. Do NOT merge, split, reorder, or rephrase strings.
4. The output MUST be a valid JSON array of strings.
5. The output array must have the same number of elements and the same order as the input.
6. If a string is empty or contains only placeholders, return it unchanged.
7. If you are unsure about a translation, return the original string unchanged.
